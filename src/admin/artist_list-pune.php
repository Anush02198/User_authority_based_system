...
echo "$target";
  	$sql = "INSERT INTO artist_list (name, bio, image, uploaded_by)VALUES ('$name','$bio','$image','$role');";
  	// execute query
  	mysqli_query($db, $sql);
  }
if($role == 'sadmin'){
  $result = mysqli_query($db, "SELECT * FROM artist_list WHERE uploaded_by IN ('sadmin', 'admin_pun')");  
}else{
  $result = mysqli_query($db, "SELECT * FROM artist_list where uploaded_by = '$role'");
}
?>
...
<div class="upload">
   <h1>UPLOAD IMAGE:</h1>
    <form method="POST" action="artist_list-pune.php" enctype="multipart/form-data">
