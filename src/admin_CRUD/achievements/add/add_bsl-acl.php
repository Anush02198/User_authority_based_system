...
<?php
	if (isset($_POST['submit'])) {
		...
		$sql = $conn->prepare("INSERT INTO awards_achievements (year,achievements,uploaded_by,division) VALUES (?,?,?,?)");  
		$year=$_POST['year'];
		$role = 'admin_bsl-acl';
		$division='bhusaval-acl';
		$achievements = $_POST['achievements'];
		$sql->bind_param("ssss", $year,$achievements, $role,$division); 
		if($sql->execute()) {
...
.tbl-qa{border-spacing:0px;border-radius:4px;border:#6ab5b9 1px solid;}
  ...
  <title>Add New Achievements</title> 	
</head>
<body>
...
<div class="button_link"><a href="../achievement_bsl-acl.php"> Back to List </a></div>
<table border="0" cellpadding="10" cellspacing="0" width="500" align="center" class="table table-striped">
	<thead class="thead-dark">
		<tr>
			<th colspan="2" class="table-header">Add New Achievement</th>
		</tr>
	</thead>
	<tbody>
		<tr class="table-row">
			<td><label>year</label></td>
			<td><input type="number" name="year" class="txtField"></td>
		</tr>
		<tr class="table-row">
			<td><label>achievements</label></td>
			<td><input type="text" name="achievements" class="txtField"></td>
		</tr>
		<tr class="table-row">
