...
header("Location: C:\xampp\htdocs\project\admin\index.php");
exit(); }
?>
<?php
  if (isset($_POST['submit'])) {    
    $sql = $conn->prepare("UPDATE deletebs SET year=? ,name=?, achievement=? WHERE id=?");
    $below_whom=$_POST['year'];
    $name = $_POST['name'];
    $sql->bind_param("ssi",$year, $name,$achievement,$_GET["id"]); 
    if($sql->execute()) {
  ...
  $sql = $conn->prepare("SELECT * FROM deletebs WHERE id=?");
...
<table border="0" cellpadding="10" cellspacing="0" width="500" align="center" class="table table-striped">
  ...
  <tbody>
    <tr class="table-row">
      ...
      <td><input type="text" name="below_whom" class="txtField" value="<?php echo $row["year]?>"></td>
    </tr>
    <tr class="table-row">
      <td><label>below_whom</label></td>
